...
class FakeDaprSidecar(api_service_v1.DaprServicer):
    def __init__(self, grpc_port: int = GRPC_PORT, http_port: int = HTTP_PORT):
        self.grpc_port = grpc_port
        self.http_port = http_port
        self._grpc_server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
        self._http_server = FakeHttpServer(
            self.http_port
        )  # Needed for the healthcheck endpoint
    ...
    def PublishEvent(self, request, context):
        ...
        if request.metadata["rawPayload"]:
            headers = headers + (
                ("metadata_raw_payload", request.metadata["rawPayload"]),
            )
        if request.metadata["ttlInSeconds"]:
            headers = headers + (
                ("metadata_ttl_in_seconds", request.metadata["ttlInSeconds"]),
            )
    ...
    def SubscribeTopicEventsAlpha1(self, request_iterator, context):
        ...
        context.abort_with_status(
            rpc_status.to_status(
                status_pb2.Status(
                    code=code_pb2.UNAVAILABLE, message="Simulated disconnection"
                )
            )
        )
    ...
    def UnlockAlpha1(self, request: UnlockRequest, context):
        ...
        if self.locks_to_owner[lock_id] == request.lock_owner:
    ...
    def GetWorkflowBeta1(self, request: GetWorkflowRequest, context):
        ...
        if instance_id in self.workflow_status:
            status = str(self.workflow_status[instance_id])[
                len("WorkflowRuntimeStatus.") :
            ]
